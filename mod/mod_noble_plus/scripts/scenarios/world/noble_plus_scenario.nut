...
this.noble_plus_scenario <- this.inherit("scripts/scenarios/world/starting_scenario", {
    ...
    function onSpawnAssets()
    {
        try
        {
            ...
            // ------------------------------------------------------------------
            // bros[0] — Дворянин-командир (барон Эдмунд Локвуд)
            // ------------------------------------------------------------------
            bros[0].getFlags().set("IsPlayerCharacter", true);
            bros[0].setStartValuesEx(["legend_noble_commander_background"], false);
            // Нарративная предыстория барона Локвуда.
            // Паттерн m.RawDescription + buildDescription(true) — единственный надёжный
            // способ задать текст: buildDescription() вызывается движком позже и перетирает
            // прямое присвоение m.Description. RawDescription — это сырой шаблон до обработки.
            bros[0].getBackground().m.RawDescription =
                "Твой отец называл тебя честью Дома Локвуд. Семь лет ты учился " +
                "быть достойным этого имени: воевал с разбойниками, судил споры " +
                "крестьян, укреплял замковые стены. " +
                "Твой брат Корвин всегда шёл к цели напрямик. Одной ночью он поднял " +
                "меч против отца — и мир перевернулся. " +
                "Теперь имя Эдмунд Локвуд — вне закона. Каждая деревня, что примет " +
                "тебя, навлечёт гнев узурпатора. Каждый, кто встанет рядом, — враг Корвина. " +
                "Но у тебя осталось имя. И пока оно звучит — надежда жива.";
