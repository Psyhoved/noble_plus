...
> Политика источников: при конфликте с `docs/MODDING_BB_LEGENDS_GUIDE.md` и external docs, приоритет у canonical-гайда.
...
### Конфиг Noble Plus (Config v1)

Начиная с текущей реализации, для `scenario.noble_plus` амбиции управляются через
`mod/mod_noble_plus/scripts/config/noble_plus_ambitions_config.nut`.

- `sets[]` определяет наборы амбиций (порядок `order`, правила завершения и события перехода).
- `ambitions[]` определяет доступные ID, тексты, thresholds, score и done-флаги.
- Любая амбиция, которой нет в активном наборе конфига, получает `Score = 0` и не появляется в пуле.
- `ambition.battle_standard` оставлена ванильной по механике, но её тексты/доступность/флаг завершения также управляются конфигом.

### Критично: деплой в live-игру перед тестом

Файлы из репозитория не применяются автоматически к `Battle Brothers/data/scripts/`.
Перед запуском игры обязательно выполнить:

```bash
./tools/deploy_live_scripts.sh
```

Скрипт копирует preload/scenario/events/ambitions в live-папку игры и проверяет SHA1.
...
this.my_ambition <- this.inherit("scripts/ambitions/ambition", {
    ...
    function create()
    {
        ...
        this.m.ButtonText = "Название/цель в окне выбора.";        // карточка выбора
