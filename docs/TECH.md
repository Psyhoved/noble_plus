...
Этот файл содержит технические детали и исторические заметки, но **не является каноническим источником правил моддинга**.
Канонический стандарт: `docs/MODDING_BB_LEGENDS_GUIDE.md`.

Если в этом файле есть противоречия с external sources (Enduriel/MSU/Modern Hooks), приоритет у canonical-гайда.
...
> Обновление: актуальные правила по ModSettings и источники вынесены в `docs/MODDING_BB_LEGENDS_GUIDE.md`.
